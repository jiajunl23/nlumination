...
export function ChatPanel({ params, onParams, className }: Props) {
  ...
  return (
    <div
      className={cn(
        "flex min-h-0 flex-col rounded-2xl border border-[--color-border] bg-[--color-bg-elev-1]",
        className,
      )}
    >
      <header className="flex items-center gap-2 border-b border-[--color-border]/60 px-4 py-3 text-sm font-medium text-[--color-fg]">
      ...
      >
        {messages.length === 0 && (
          <div className="rounded-lg border border-dashed border-[--color-border] bg-[--color-bg-elev-2]/40 p-3 text-xs text-[--color-fg-muted]">
            Type a prompt like{" "}
        ...
        {messages.map((m) =>
          m.role === "user" ? (
            <div key={m.id} className="flex justify-end">
              <div className="max-w-[85%] rounded-2xl bg-[--color-bg-elev-3] px-3 py-2 text-[--color-fg]">
                {m.text}
              </div>
            </div>
          ) : (
            <div key={m.id} className="max-w-[95%] space-y-1.5">
              {m.applied.length > 0 ? (
                <div className="text-[--color-fg-muted]">
                  <span className="text-[--color-fg-dim]">applied:</span>{" "}
                  <span className="text-[--color-fg]">
                    {m.applied.join(", ")}
                  </span>
                </div>
              ...
              {m.hint && (
                <div className="text-xs italic text-[--color-fg-dim]">
                  → {m.hint}
                </div>
              )}
                ...
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {m.tryChips.map((s, i) => (
                    <button
                      ...
                      className="rounded-md border border-[--color-border] bg-[--color-bg-elev-2] px-2 py-0.5 text-[11px] text-[--color-fg] transition hover:border-[--color-border-strong] hover:bg-[--color-bg-elev-3]"
                    >
                      {s.phrase}
                    </button>
                  ))}
      ...
      <div className="space-y-2 border-t border-[--color-border]/60 p-3">
        <form
          onSubmit={(e) => {
            e.preventDefault();
            submit(value);
          }}
          className="flex items-center gap-2 rounded-full border border-[--color-border] bg-[--color-bg-elev-2]/60 px-3 py-2 transition focus-within:border-[--color-border-strong]"
        >
          <input
            type="text"
            value={value}
